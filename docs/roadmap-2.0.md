...
11. **Menú "Más" de la terminal en MD3 Expressive** — el menú "Más" actual del terminal es un `ContextMenu` clásico de Android (`TermuxComposeActivity.onCreateContextMenu`/`onContextItemSelected`, líneas ~954–1039): Seleccionar URL, compartir transcripción/texto, autofill, reiniciar terminal, matar proceso, Estilo, mantener pantalla encendida, ayuda, ajustes, reportar issue; se abre desde el "Más" de la toolbar de selección de texto (`ComposeTerminalSelectionOverlay` → `text_selection_more`). Reemplazarlo por un menú Compose M3 Expressive (`ModalBottomSheet` o `DropdownMenu` bajo `MaterialExpressiveTheme`, paridad de ítems y checks como "Mantener pantalla encendida"), disparado desde la toolbar de selección y/o un `MoreVert` en la top bar de `TermuxMainScreen`. Mantener la misma lógica de handlers (mover `onContextItemSelected` a callbacks Compose/ViewModel); el file manager y About ya usan `DropdownMenu` M3 y pueden servir de patrón. Riesgo medio: toca flujo de selección compartido legacy/nativo.
12. **Filemanager — Editar en la terminal** — acción «Editar» para abrir el archivo con un editor del rootfs Debian (`nano`, `vim`, `vi`, `vim.tiny`, `mcedit`, `ed` — detectados con probe en `TermuxConstants.DEBIAN_ROOTFS_DIR_PATH/usr/bin/*` + `canExecute()`; rootfs minbase **no trae editores** → diálogo/snackbar `filemanager_no_editor` con hint `apt install nano`). UI en el módulo `filemanager` (biblioteca autocontenida, sin conocimiento de sesiones): callback opcional `onEditFile: ((File) -> Unit)?` junto a `onOpenInTerminal` (`FileManagerScreen.kt:116`); botones en la **barra de selección** (tras Share, ~línea 291) y en el **diálogo de detalles** (Row de Copy path/Rename, ~línea 632); strings EN/ES/PT (`action_edit`, `filemanager_no_editor`, `filemanager_choose_editor`, `filemanager_unmappable_path`); resolver symlinks con `resolveFileForOpen`; solo archivos (ocultar en directorios). Cableado en `app` igual que `onOpenInTerminal`: `FileManagerSessionHost` → `TermuxMainScreen` → `TermuxComposeActivity.openFileInEditor(hostPath)` (hermano de `openTerminalIn` ~715): guards MAX_SESSIONS + `DebianInstaller.isInstalled()` + file exists + path mapeable vía `ProotShellEnvironment.hostPathToGuestPath` (rootfs/`/sdcard`/`/storage`; resto → toast no visible en Debian); 1 editor → lanza ya; N editors → `AlertDialog` de elección (lógica app-side); 0 → aviso + hint apt. Sesión: `service.createTermuxSession(null, arrayOf("-c", "<editor> '<guest>'; exec bash"), null, parentDirHost, false, null)` — path guest entrecomillado (escape `'` → `'\''`) y **`; exec bash`** para que la pestaña quede como shell al salir del editor (decidido); `mViewModel.addSession(…, "nano: nombre")`. **No** usar RUN_COMMAND/`ACTION_SERVICE_EXECUTE` (validan executable host + flag plugin desactiva el branch proot). `FileManagerComposeActivity` (legacy) deja `onEditFile = null` (oculto, paridad con `onOpenInTerminal` ya omitido). Cierra la promesa de fastlane *"Edit files with nano and vim"*.

### Fase 2.5 — Rendimiento de plataforma (medio riesgo)

13. **Tasa de refresco máxima (90/120Hz)** — no existe ningún uso de `preferredDisplayModeId`/`preferredRefreshRate`/`setFrameRate` en el repo. En `TermuxComposeActivity.onCreate`: leer `Display.getSupportedModes()`, elegir el modo con mayor `refreshRate` (o `display.mode` con resolución nativa + rate máximo), aplicar `window.attributes.preferredDisplayModeId` + `preferredRefreshRate`. Fallback silencioso si falla (modo default). Opcional: tile en Ajustes para forzar 60Hz (batería). Hardware acceleration ya está ON por defecto (no hay `android:hardwareAccelerated` en el manifest).
14. **Beneficio del hardware por dispositivo** — hoy no hay detección adaptativa (`Build.*` solo se usa en reportes de bug vía `AndroidUtils.getDeviceInfoMarkdownString`). Detectar RAM total (`ActivityManager.getMemoryInfo`), cores y `Build.HARDWARE` para calcular defaults: tamaño de scrollback, pref de renderer nativo, agresividad del blink, límites de listas del file manager. Implementar como preferencias con **default calculado al primer arranque** (no fuerza cambios a instalaciones existentes). No romper el modelo de flags actual.
15. **btop: `Failed to parse /proc/stat` — limitación documentada (sin código)** — el bind `-b /proc` en `ProotShellEnvironment.buildProotCommand()` ya es correcto; falla porque SELinux deniega a apps `untrusted_app` leer `/proc/stat` desde Android 8/API 26 (misma clase de restricción ya documentada para `/dev` y `/sys` en `README.md` y `docs/proot-debian-arm64-plan.md`). Sin workaround sin root/SELinux permissive. Añadir nota en README/docs: monitores CPU (`btop`, `htop`) no mostrarán stats de CPU en stock enforcing.
