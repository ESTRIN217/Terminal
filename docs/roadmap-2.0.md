...
6. **Terminal nativa pintada con Compose** — spike operativo (working tree, pendiente de commit): `ComposeTerminalCanvas` pinta la sesión con un `Canvas` Compose en lugar de la vista legada, con la lógica pura de runs en `ComposeTerminalFrame` (agrupación por estilo, reverse video, cursor block/underline/bar, caracteres anchos y combining, escala de glifos no-mono). Detrás del **feature-flag** `native_compose_renderer` (Ajustes → Experimental) con la vista legada como fallback, tal como exige el roadmap. El input vive en `HiddenTerminalInputHost` (vista oculta full-size que posee IME/teclas/foco; el offset de scroll lo posee el canvas), ambos con la misma geometría (`updateSize`). Gestos con paridad `TerminalView.doScroll`: swipe abajo = volver (transcript), swipe arriba = live; en alt-buffer manda flechas UP/DOWN; con **mouse-tracking activo** (TUIs como opencode) manda eventos de rueda anclados a la posición del dedo. Tests de la lógica en `ComposeTerminalFrameTest` (Kotlin + Java). Diferido: selección de texto, cursor parpadeante (se pinta estático), pinch-zoom y fling con inercia.
...
| Fase 3 — Rendering | En curso (spike nativo Compose operativo tras el flag) |
