...
7. **Hipervínculos (OSC 8) e imágenes** — ✅ hecho: solo OSC 8 (**imágenes descartadas — decisión registrada**). `TerminalEmulator` parsea `ESC ] 8 ; params ; URI BEL|ST` (params como `id=` se ignoran en v1), registra URIs en un índice 1-based (0 = sin link) y sella `TerminalRow.mHyperlinkIds` (side-band) en `emitCodePoint` (ambas columnas de caracteres anchos); `getHyperlinkUriAt` / `isHyperlinkActive` / allowlist estático `isAllowedHyperlinkUri` (`http/https/ftp/file/gemini/mailto`, rechaza `javascript:`). Renders: **legacy** `TerminalRenderer` rompe el run al cambiar el link y fuerza underline; **Compose** `TextRun.hyperlinkIndex` + break en `buildLineRuns` + underline en `drawComposeRun`. Tap: guards pref ON, sin mouse-tracking, sin selección; legacy `TerminalView.onSingleTapUp` → `ACTION_VIEW`, Compose `onTap` → `ShareUtils.openUrl`. Pref experimental `terminal_hyperlinks` (default `true` kill-switch) con receta de 5 capas + tile `[Experimental]` EN/ES (PT sin cadenas, cae al default). Tests: `HyperlinkTest` (8) + `ComposeTerminalFrameTest.testHyperlinkChange_splitsRuns`.
